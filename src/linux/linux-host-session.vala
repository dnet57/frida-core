	...
	private class CrashMonitor : Object {
		...
		private void on_log_entry (LogEntry entry) {
			...
			if (is_java_crash) {
				try {
					...
					printerr ("parse_java_report() failed: %s\n%s\n", e.message, entry.message);
				}
		...
		private void on_crash_builder_completed (CrashBuilder builder, string report) {
			...
			try {
				...
				printerr ("parse_native_report() failed: %s\n%s\n", e.message, report);
			}
		}
		...
		private static CrashInfo parse_native_report (string report) throws Error {
			MatchInfo info;
			if (!/^pid: (\d+), tid: \d+, name: (.+) +>>>/m.match (report, 0, out info)) {
		...
		private class CrashBuilder : Object {
			...
			private bool on_poll_tick () {
				...
				log_event ("[reporter_pid %u] reporter_still_alive=%s", reporter_pid, reporter_still_alive.to_string ());
			...
			private void schedule_completion () {
				log_event ("[reporter_pid %u] scheduling completion", reporter_pid);
			...
			private void defer_completion () {
				...
				log_event ("[reporter_pid %u] deferring completion", reporter_pid);
			...
			private bool on_complete () {
				log_event ("[reporter_pid %u] complete", reporter_pid);
